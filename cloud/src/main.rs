...
fn build_ui(app: &Application) {
    ...
    let clients = backend::csvreader::create_client_secret_csv("../clientsecrets").unwrap();

    //let mut user = Arc::new(Mutex::new(tokio::runtime::Runtime::new().unwrap().block_on(backend::youtubeapi::User::new(clients.to_str().unwrap()))));

    let user: Arc<Mutex<Option<backend::youtubeapi::User>>> = Arc::new(Mutex::new(None));

    let user_clone = user.clone();
    let clients_str = clients.to_str().unwrap().to_string();

    MainContext::default().spawn_local(async move {
        // Fill in the Option<User> later using spawn_local
        let mut user = user_clone.lock().unwrap();
        *user = Some(backend::youtubeapi::User::new(&clients_str).await);
    });
    ...
    file_button.connect_clicked(move |_btn| {
        ...
        file_chooser.connect_response(move |dialog, response| {
            if response == ResponseType::Accept {  
                ...
                let mut user_clone2 = user_clone.clone();
                println!("{}", user_clone.clone().lock().unwrap().as_mut().unwrap().name);
                glib::MainContext::default().spawn_local(async move {
                    match user_clone2.lock().unwrap().as_mut().unwrap().upload(file_path.to_str().unwrap()).await {
