...
impl User {
    ...
    pub async fn new(path: &str) -> Result<User, Box<dyn Error + Send + Sync>> {
...
impl User {
    ...
    pub async fn upload(&mut self, file_path: &str) -> Result<(), Box<dyn Error + Send + Sync>>{
...
impl Api {
    ...
    async fn new(path: &str) -> Self {
        ...
        Api { client: path.to_string(), quota, token: token.clone(), expiration_time, path: Path::new(path).to_path_buf()}
    }

    /// Returns the authenticated Youtube Data API v3 hub.
    fn hub(&self) -> YouTube<hyper_rustls::HttpsConnector<HttpConnector>> {
        return self.build_hub();
    }

    fn build_hub(&self) -> YouTube<hyper_rustls::HttpsConnector<HttpConnector>> {
        let https = hyper_rustls::HttpsConnectorBuilder::new()
        .with_native_roots()
        .https_only()
        .enable_http1()
        ...
        let hub = YouTube::new(
            hyper::Client::builder().build::<_, hyper::Body>(https),
            self.token.token().clone().unwrap().to_string(),
        );
